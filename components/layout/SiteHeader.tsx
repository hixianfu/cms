...
export function SiteHeader({ locale, global, categories = [], megaMenu }: Props) {
  ...
  return <header className={headerClass}>
    <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      <Link href={localizedHref(locale, "/")} className={`flex items-center gap-3 ${overlayHero ? "text-white" : "text-brand-blue"}`}>
        {logo ? <img src={logo} alt={global?.logoAlt ?? global?.siteName ?? ""} className={`h-10 w-auto ${overlayHero ? "brightness-0 invert" : ""}`} /> : <span className="text-lg font-bold">{global?.siteName ?? "Ameson"}</span>}
      </Link>
      <nav className="hidden items-stretch gap-7 self-stretch lg:flex" aria-label="Primary">
        {items.map((item, index) => {
          const menu = menuFor(item.href);
          const linkClass = overlayHero
        ...
        <Link href={localizedHref(locale, "/search")} aria-label={locale === "zh" ? "全站搜索" : "Site search"} className={`my-auto rounded-lg p-2 transition ${overlayHero ? "text-white hover:bg-white/10 hover:text-white/80" : "text-brand-ink hover:bg-brand-teal/10 hover:text-brand-blue"}`}><Search size={18} /></Link>
        <div className="my-auto"><LocaleSwitcher locale={locale} inverse={overlayHero} /></div>
      </nav>
      <div className="flex items-center gap-2 lg:hidden">
        <Link href={localizedHref(locale, "/search")} aria-label={locale === "zh" ? "全站搜索" : "Site search"} className={`rounded-lg p-2 ${overlayHero ? "text-white hover:bg-white/10" : "text-brand-blue"}`}><Search size={20} /></Link>
        <LocaleSwitcher locale={locale} inverse={overlayHero} />
        <button type="button" className={`rounded-lg p-2 ${overlayHero ? "text-white hover:bg-white/10" : "text-brand-blue"}`} onClick={() => setOpen(true)} aria-expanded={open} aria-label={messages[locale].menu}><Menu size={22} /></button>
      </div>
    </div>
