...
export function MegaMenu({ locale, menu }: { locale: Locale; menu: ProductMenu | ContentMenu }) { return <div className="invisible absolute inset-x-0 top-full z-50 border-t border-brand-border bg-white opacity-0 shadow-[0_18px_40px_rgba(18,50,74,0.14)] transition duration-200 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100"><div className="mx-auto max-h-[70vh] max-w-7xl overflow-y-auto px-8 py-10">{menu.kind === "products" ? <ProductMegaMenu locale={locale} categories={menu.categories} /> : <ContentMegaMenu locale={locale} path={menu.path} items={menu.items} />}</div></div>; }
