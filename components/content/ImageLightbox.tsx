...
export function ImageLightbox({ images, initialIndex = 0, className = "" }: { images: LightboxImage[]; initialIndex?: number; className?: string }) {
  ...
  return <>
    ...
    {open ? <div role="dialog" aria-modal="true" aria-label="Image preview" className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 sm:p-8" onClick={(event) => { if (event.target === event.currentTarget) close(); }} onWheel={(event) => { event.preventDefault(); setZoom((value) => Math.min(4, Math.max(1, value + (event.deltaY < 0 ? 0.2 : -0.2)))); }}>
