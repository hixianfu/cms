...
export function ProductImageGallery({ cover, gallery, locale, productName }: { cover?: Media | null; gallery?: Media[] | null; locale: string; productName: string }) {
  ...
  return <section className="w-full min-w-0 max-w-full overflow-hidden" aria-label={zh ? "产品图片" : "Product images"}>
    <button type="button" onClick={() => setPreviewOpen(true)} onMouseMove={updateZoomOrigin} onMouseLeave={() => setZoomOrigin("50% 50%")} className="group relative block w-full max-w-full cursor-zoom-in overflow-hidden rounded-2xl border border-brand-border bg-slate-100 shadow-sm" aria-label={zh ? "查看大图" : "View larger image"}>
    ...
    {items.length > 1 ? <div className="mt-4 flex max-w-full gap-3 overflow-x-auto pb-2">{items.map(({ media, url }, index) => <button type="button" key={url} onClick={() => setActive(index)} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 bg-slate-100 transition sm:h-20 sm:w-20 ${index === active ? "border-brand-teal ring-2 ring-brand-teal/20" : "border-transparent hover:border-brand-border"}`} aria-label={`${zh ? "图片" : "Image"} ${index + 1}`} aria-current={index === active ? "true" : undefined}><Image src={url} alt={media.alternativeText ?? ""} fill sizes="80px" className="object-cover" /></button>)}</div> : null}
