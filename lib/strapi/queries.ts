...
export async function getProductCategories(locale?: Locale) { const r = await strapiFetch<Response<ProductCategory>>(`/api/product-categories${collectionParams(locale)}&sort=sortOrder:asc,name:asc`, { next: { revalidate: 300, tags: [cacheTags.products(tagLocale(locale))] } }); return Array.isArray(r.data) ? r.data.map((x) => unwrap(x)) : []; }
...
export async function getAbout(locale?: Locale) { const r = await strapiFetch<Response<About>>(`/api/about${params(locale)}`, { next: { revalidate: 300, tags: [cacheTags.about(tagLocale(locale))] } }); return r.data ? unwrap(r.data as Entity<About>) : null; }
export async function getContactPage(locale?: Locale) { const r = await strapiFetch<Response<ContactPage>>(`/api/contact-page${params(locale)}`, { next: { revalidate: 300, tags: [cacheTags.contactPage(tagLocale(locale))] } }); return r.data ? unwrap(r.data as Entity<ContactPage>) : null; }
