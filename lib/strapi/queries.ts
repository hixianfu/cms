...
export async function getHomePage(locale?: Locale) { const r = await strapiFetch<Response<HomePage>>(`/api/home-page${params(locale)}`, { next: { revalidate: 120, tags: [cacheTags.homePage(tagLocale(locale))] } }); return r.data ? unwrap(r.data as Entity<HomePage>) : null; }
