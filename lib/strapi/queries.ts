...
import type { About, Article, ArticleCategory, CaseStudy, ContactPage, Global, HomePage, Locale, Product, ProductCategory, Video } from "@/types/content";
...
export async function getArticles(locale?: Locale, query = "") { const params = `?populate[cover]=true&populate[author]=true&populate[category]=true&populate[products]=true${locale ? `&filters[contentLocale][$eq]=${encodeURIComponent(locale)}` : ""}${query ? `&${query.replace(/^\?/, "")}` : ""}&sort=publishedAt:desc`; const r = await strapiFetch<Response<Article>>(`/api/articles${params}`, { next: { revalidate: 60, tags: [cacheTags.articles(tagLocale(locale))] } }); return Array.isArray(r.data) ? r.data.map((x) => unwrap(x)) : []; }
