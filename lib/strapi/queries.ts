...
export async function getArticles(locale?: Locale, query = "") { const r = await strapiFetch<Response<Article>>(`/api/articles${collectionParams(locale, `${query ? `&${query.replace(/^\?/, "")}` : ""}&populate[products]=true`)}&sort=publishedAt:desc`, { next: { revalidate: 60, tags: [cacheTags.articles(tagLocale(locale))] } }); return Array.isArray(r.data) ? r.data.map((x) => unwrap(x)) : []; }
