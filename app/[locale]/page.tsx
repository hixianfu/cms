...
export default async function LocaleHome({ params }: { params: Promise<{ locale: string }> }) { const { locale } = await params; if (!isLocale(locale)) notFound(); const home = await getHomePage(locale); const current = locale as Locale; return <div>{home?.introTitle || home?.intro ? <section className="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6"><h2 className="text-3xl font-semibold text-slate-950">{home.introTitle ?? (locale === "zh" ? "关于我们" : "About us")}</h2>{home.intro ? <p className="mt-4 text-lg leading-8 text-slate-600">{home.intro}</p> : null}</section> : null}{home?.sections?.map((section, index) => renderSection(section, current, index))}<div className="mx-auto max-w-7xl px-4 sm:px-6">{home?.blocks?.length ? <section className="py-12"><RichTextRenderer blocks={home.blocks} /></section> : null}</div><CtaSection locale={current} cta={home?.cta} /></div>; }
