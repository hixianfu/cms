...
export default async function CasesPage({ params, searchParams }: { params: Promise<Params>; searchParams: Promise<Search> }) {
  ...
  const { q, category, industry, product, scenario } = await searchParams; const query = [q?.trim() ? `filters[title][$containsi]=${encodeURIComponent(q.trim())}` : "", category ? `filters[category][slug][$eq]=${encodeURIComponent(category)}` : "", industry ? `filters[industry][$eq]=${encodeURIComponent(industry)}` : "", product ? `filters[products][slug][$eq]=${encodeURIComponent(product)}` : "", scenario ? `filters[scenarios][slug][$eq]=${encodeURIComponent(scenario)}` : ""].filter(Boolean).join("&");
